...
`token_embd.weight`. The first hybrid backend improved the local 2-token smoke
from about 44 seconds to about 37 seconds; the optimized quant matvec path brings
that down again to about 28 seconds. TinyLlama remains best on CPU because it is
too small to amortize the GPU work. Mistral 7B changed materially after the
engine improvements: the first hybrid smoke regressed to about 102 seconds, while
the optimized hybrid path is about 62 seconds and is now the fastest tested mode
for that model on this machine.
...
These rows are also folded into the main results table as the "Optimized engine
result" column:
