...
| `RBITNET_KV_POOL` | `0` | **`1`** — process-wide **shared physical page pool** (PagedAttention-style). Llama runtime KV is backed by `SharedPhysKvStore`; pages return to the free list on request `clear()` / sequence close. Implies paged mode. Metrics: `rbitnet_core_kv_pool_*`. Enable for concurrency benches: `scripts/bench_paged_kv.sh`. `rbitnet tune throughput` sets this. |
| `RBITNET_PAGED_KV_PAGE_TOKENS`, `RBITNET_PAGED_KV_MAX_PAGES` | `16`, `4096` | Page size and per-layer physical page cap when **`RBITNET_LLAMA_PAGED_KV`** or **`RBITNET_KV_POOL`** is enabled; also tune experimental Qwen35 attention metadata (see code). |
