...
3. **Convert to GGUF** using a pipeline that targets **llama** architecture in GGUF:
   - **BitNet / Microsoft tooling** for BitNet-specific checkpoints ([MODEL_TESTING.md](MODEL_TESTING.md)).
   - **`llama.cpp` `convert_*.py`** or **`llama.cpp`-compatible** exporters for standard Llama-family models.
...
For checkpoints published as **1.58-bit / BitNet** (for example [1bitLLM/bitnet_b1_58-large](https://huggingface.co/1bitLLM/bitnet_b1_58-large)), conversion to GGUF is **not** done by Rbitnet. Follow **Microsoft BitNet** documentation: Python environment, `setup_env.py`, and their export scripts produce a `.gguf` with the right tensor packing for their stack. Rbitnet then reads that GGUF if types and names match its loader.
...
| Export to GGUF | BitNet scripts, llama.cpp converters, etc. — **often Python** |
