...
| **B — Scheduling** | `run_batch` / `run_batch_waves`; `PrefillDecodeQueue`; `RBITNET_CONTINUOUS_BATCHING` interleaved decode; chunked prefill env | Fused multi-seq matmul; **stall-free chunked prefill** ([2403.02310](https://arxiv.org/abs/2403.02310)) |
| **C — Cache semantics** | Dense prefix KV (`RBITNET_PREFIX_KV`); paged snapshots; radix `longest_token_prefix`; HTTP sidecar PUT/GET | **LRU radix + `prefix_hit` metrics** ([2312.07104](https://arxiv.org/abs/2312.07104)); L7 sticky |
