...
pub fn token_embedding_row(
    ...
    if t.dimensions.len() != 2 {
        return Err(BitNetError::Inference("token_embd: expected dims [embd,vocab]".into()));
    }
    let dim0 = usize::try_from(t.dimensions[0]).map_err(|_| BitNetError::Inference("ne0 overflow".into()))?;
    let dim1 = usize::try_from(t.dimensions[1]).map_err(|_| BitNetError::Inference("ne1 overflow".into()))?;
    if dim0 != n_embd || dim1 != n_vocab || token_id >= n_vocab {
        return Err(BitNetError::Inference("token_embd shape mismatch vs config".into()));
    }
    let row_bs = ggml_row_size(t.ggml_type, t.dimensions[0])?;
    let row_start = token_id.checked_mul(row_bs).ok_or_else(|| BitNetError::Inference("embed offset overflow".into()))?;
...
pub fn quant_matmul_vec(payload: &[u8], ggml_ty: u32, ne0: usize, ne1: usize, x: &[f32]) -> Result<Vec<f32>> {
    if x.len() != ne0 {
    ...
    if need != payload.len() && ne1 != 0 {
        ...
        if payload.len() < need {
            return Err(BitNetError::Inference("quant matmul truncated payload".into()));
        }
    }
...
pub fn quant_matmul_vec_offset(
    ...
    if offset.checked_add(total).filter(|e| *e <= payload.len()).is_none() {
